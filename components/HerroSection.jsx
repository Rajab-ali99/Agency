...
export default function HeroSection() {
  ...
  return (
    <div className="relative md:h-[115vh] h-[100vh] flex items-center justify-center overflow-hidden bg-[#0a1412]">
      ...
      {/* 🌀 Middle Ring — rotates, but images stay upright */}
      <motion.div
        style={{ rotate: combinedRotation }}
      ...
      >
        {Array.from({ length: 20 }).map((_, i) => {
          ...
          return (
            ...
            >
              {/* Inner fix: cancel ring rotation */}
              <motion.div style={{ rotate: useTransform(combinedRotation, (r) => -r) }}>
                <Image
                  src={middleRingImages[i % middleRingImages.length]}
                  alt={`middle-ring-${i}`}
                  width={110}
                  height={110}
                  className="rounded-full border border-white/20 object-cover h-full w-full shadow-[0_0_15px_rgba(255,255,255,0.1)]"
                />
              </motion.div>
            </div>
          );
      ...
      {/* 🌀 Inner Ring — rotates, but images stay upright */}
      <motion.div
        style={{ rotate: combinedRotation }}
            ...
            >
              ...
              style={{ rotate: useTransform(combinedRotation, (r) => -r) }}>
                <Image
                  ...
                  width={80}
                  height={80}
                  className=" rounded-full border border-white/20 object-cover h-full w-full shadow-[0_0_10px_rgba(255,255,255,0.1)]"
                />
              </motion.div>
            </div>
          );
